...
logging.basicConfig(
    format=(
        '%(asctime)s - %(name)s: [%(levelname)s - Science Museum UK] '
        '=======> %(message)s'),
    level=logging.INFO)
...
class ScienceMuseum(Provider):
    ...
    def getMetaData(self, _html, _url):
        ...
        # verify the license
        # soup.find('svg', {'class': 'icon icon-cc-zero'})
        licenseDetails = soup.find('div', {'class': 'cite__method'})
        if licenseDetails:
            imgLicense = (
                licenseDetails.findChild('img'))  # licenseDetails.parent
            if imgLicense and 'src' in imgLicense.attrs:
                imgLicense = self.validateContent(
                    '', imgLicense, 'src').split('/')
                imgLicense = imgLicense[len(imgLicense)-1].split(
                    '.')[0].lstrip('cc-')
        ...
        if description:
            otherMetaData['description'] = self.sanitizeString(
            ...
            if maker:
                ...
                if makerName.lower() != 'unknown':
                    ...
                    if 'href' in maker.attrs:
                        self.creatorURL = self.validateContent(
                            '', maker, 'href')

        # meta data
        timeline = soup.find('dl', {'class': 'record-top__dl fact-Made'})
        if timeline:
            timeline = timeline.text.strip().replace(
                'Made:', '').replace('Maker:', '').split('in')
            if len(timeline) > 1:
                otherMetaData['date'] = timeline[0].strip()
                otherMetaData['geography'] = timeline[1].strip()

        otherDetails = soup.find_all(
            'dl', {'class': re.compile(r'(record-details.*?)')})
        if otherDetails:
            for detail in otherDetails:
                key = detail.findChild(
                    'dt').text.strip().lower().replace(' ', '_')
                key = key.rstrip(':')
                val = self.sanitizeString(
                    detail.findChild('dd').text.strip())
