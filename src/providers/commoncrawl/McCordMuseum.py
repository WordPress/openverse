"""
Content Provider:       McCord Museum - Collections of artwork that celebrates
                        Canadian History.

ETL Process:            Identify images from their art collection that are
...
logging.basicConfig(
    format=(
        '%(asctime)s - %(name)s: [%(levelname)s - McCord Museum] '
        '=======> %(message)s'),
    level=logging.INFO)
...
class McCordMuseum(Provider):
    ...
    def getMetaData(self, _html, _url):
        ...
        if imgContent:
            ...
            # removed because of inconsistent dimensions.
            # self.url = self.url.replace('/ObjView/', '/largeimages/')
            ...
            try:
                ...
                logging.warning(
                    f'Error extracting the image dimensions => {imgSRC}')
            ...
            if imgAltText:
                otherMetaData['image_alt_text'] = (
                    self.sanitizeString(imgAltText))
        ...
        if otherInfo:
            artisrtInfo = otherInfo.findChild(
                'a', {'href': re.compile('.*?tablename=artist.*?')})
            if artisrtInfo:
