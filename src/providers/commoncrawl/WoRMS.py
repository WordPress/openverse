...
logging.basicConfig(
    format=(
        '%(asctime)s - %(name)s: [%(levelname)s - WoRMS] '
        '=======> %(message)s'),
    level=logging.INFO)
...
class WoRMS(Provider):
    ...
    def getMetaData(self, _html, _url):
        ...
        Returns
        ------------------
        A tab separated string which contains the meta data that was
        ...
        if urlInfo:
            ...
            foreignID = (parse_qs(foreignURL.query)['pic'],
                         urlInfo.attrs['data-url'])
        ...
        if 'p=image' in _url:
            ...
            try:
                ...
                logging.warning(
                    f'Error extracting the image dimensions => {imgDetails}')
            ...
            # title = soup.find(
                # 'div', {'class': 'photogallery_caption photogallery_title'})
            # if title:
            ...
            if desc:
                descText = desc.findChild(
                        'span',
                        {'class': 'photogallery_caption photogallery_text'})
                if descText and descText.text.strip():
                    otherMetaData['description'] = (
                        self.sanitizeString(descText.text.strip()))

            authorInfo = soup.find(
            ...
            if authorInfo:
                ...
                if author:
                    ...
                    author = authorInfo.findChild(
                        'span',
                        {'class': 'photogallery_caption photogallery_text'})
                    if author and author.text.strip():
                        self.creator = author.text.strip()

            # taxa temporarily excluded. That info may not be from a verifiable
            # source.
            ...
            return None
            # unable to verify the image license from this page (only text)
