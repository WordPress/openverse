...
ETL Process:            Identify images of vacation rentals that are available
...
logging.basicConfig(
    format=(
        '%(asctime)s - %(name)s: [%(levelname)s - IHA] =======> %(message)s'),
    level=logging.INFO)
    ...
    def getMetaData(self, _html, _url):
        ...
        if slider:
            ...
            for imageData in content:
                ...
                # get the license
                licenseInfo = (
                    imageData.find('a', {'rel': 'license', 'href': True}))
                if licenseInfo:
                ...
                if tags:
                    otherMetaData['tags'] = (
                        self.validateContent('', tags, 'content'))
