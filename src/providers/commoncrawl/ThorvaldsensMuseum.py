...
logging.basicConfig(
    format=(
        '%(asctime)s - %(name)s: [%(levelname)s - Thorvaldsens Museum] '
        '=======> %(message)s'),
    level=logging.INFO)
...
class ThorvaldsensMuseum(Provider):
    ...
    def getMetaData(self, _html, _url):
        ...
        if not imageURL:
            ...
            if imgDimensions:
                ...
                self.title = self.sanitizeString(
                    self.validateContent('', imgDimensions, 'alt'))

        # get title info
        # titleInfo = soup.find('a', {'class': 'enlarge'})
        # if titleInfo:
            # self.title = self.validateContent('', titleInfo, 'title')
            # self.foreignIdentifier = self.validateContent(
            #   '', titleInfo, 'data-id').lstrip('work_resource_')
        ...
        if artistInfo:
            ...
            self.creatorURL = self.validateContent('', artistInfo.findChild(
                'a', {'class': 'standard', 'href': True}), 'href')
