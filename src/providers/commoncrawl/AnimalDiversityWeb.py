...
logging.basicConfig(
    format=(
        '%(asctime)s - %(name)s: [%(levelname)s] - Animal Diversity Web '
        '=======> %(message)s'),
    level=logging.INFO)
...
class AnimalDiversityWeb(Provider):
    ...
    def filterData(self, _data, _condition=None):
        # Images can be located in three main content paths:
        # /accounts, /collections, and /site.
        allowed = list(map(
            lambda x: f'{self.domain}{x}',
            ['/accounts/', '/collections/', '/site/']))
        data = list(filter(
    ...
    def getMetaData(self, _html, _url):
        ...
        if licenseInfo:
            ...
            if imgProperty:
                ...
                self.width = (
                    self.validateContent('', imgProperty, 'data-width'))
                self.height = (
                    self.validateContent('', imgProperty, 'data-height'))
                otherMetaData['image_alt_text'] = (
                    self.validateContent('', imgProperty, 'alt'))
            ...
            keywords = soup.find_all(
                'ul', {'class': re.compile('keywords( last)?')})
            tagsList = []
            for tag in keywords:
                element = tag.find('li', {'class': None})
                if element:
                    tagsList.extend(
                        ele.strip() for ele in element.text.split('::'))
