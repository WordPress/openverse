...
class Provider:
    ...
    def __init__(self, _name, _domain, _cc_index):
        """
        Initialize a Provider with their name, domain and the common crawl
        index that contains their data. Subsequently, validate the common crawl
        index parameter before initializing the job.
    ...
    def __str__(self):
        ...
        return '\t'.join([
            self.imageMetaData[key] if self.imageMetaData[key] is not None else
    ...
    def getTags(self):
        ...
        if 'tags' in self.metaData:
            ...
            return [
                {'name': self.sanitizeString(tag), 'provider': self.provider}
                for tag in list(set(self.tags.split(',')))[:maxTags]]
        else:
            return self.tags
    ...
    def formatOutput(self):
        ...
        if self.metaData:
            self.metaData.update(
                (key, self.sanitizeString(val) if isinstance(val, str) else
                 val) for key, val in self.metaData.items())
    ...
    def getLicense(self, _domain, _path, _url):
        ...
        if 'creativecommons.org' not in _domain:
            logging.warning(
                (f'The license for the following work -> {_url} is not issued '
                 f'by Creative Commons.'))
            return [None, None]

        pattern = (
            re.compile('/(licenses|publicdomain)/([a-z\-?]+)/(\d\.\d)/?(.*?)'))
    ...
    def getData(self):
        ...
        providerDF = dataDF.select(
            concat(
                concat('provider_domain', 'content_path'),
                when(
                    col('content_query_string') != '',
                    concat(
                        lit('?'),
                        col('content_query_string'))).otherwise(lit(
                            ''))).alias('url'),
            concat(
                'warc_segment', lit('/warc/'),
                'warc_filename').alias('warc_filename'),
            'content_offset', 'deflate_length') \
            .where(col('provider_domain').like(f'%{self.domain}')) \
            .dropDuplicates(['url'])

        # convert dataframe into a list of tab delimited elements
        providerData = providerDF.rdd.map(
            lambda row: '\t'.join([str(col) for col in row])).collect()
    ...
    def filterData(self, _data, _condition=None):
        ...
        if _condition:
            data = filter(
                lambda x: _condition.lower() in x.split('\t')[0].lower(),
                _data)
            self.data = list(data)
        else:
            self.data = _data
    ...
    def saveData(self, _data):
        ...
        dataDF = _data.toDF([
            'foreign_identifier', 'foreign_landing_url', 'url', 'thumbnail',
            'width', 'height', 'filesize', 'license', 'license_version',
            'creator', 'creator_url', 'title', 'meta_data', 'tags',
            'watermarked', 'provider', 'source'])
        ...
        # write to file
        result.coalesce(1).write.mode('append').options(sep='\t', quote='')\
              .csv(self.output)
    ...
    def extractHTML(self, _iter):
        ...
        for row in _iter:
            ...
            content = self.getWARCRecord(
                data[1].strip(), data[2].strip(), data[3].strip())
