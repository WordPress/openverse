...
logging.basicConfig(
    format=(
        '%(asctime)s - %(name)s: [%(levelname)s - Behance] '
        '=======> %(message)s'),
    level=logging.INFO)
...
class Behance(Provider):
    ...
    def getMetaData(self, _html, _url):
        ...
        if not licenseInfo:
            licenseInfo = soup.find(
                'div', {'class': 'ProjectCopyright-tooltipContent-LVf'})
        ...
        if title:
            self.title = self.sanitizeString(
                self.validateContent('', title, 'content'))

        # url
        foreign_url = soup.find('meta', {'property': 'og:url'})
        if foreign_url:
            self.foreignLandingURL = (
                self.validateContent(_url, foreign_url, 'content'))

        # creator info
        owner = soup.find('meta', {'property': 'og:owners'})
        if owner:
            self.creator = self.sanitizeString(
                self.validateContent('', owner, 'content'))

        creators = soup.find_all('div', {'class': 'rf-profile-item__info'})

        if not creators:
            creators = soup.find_all(
                'div', {'class': 'ProjectOwnersInfo-userInfo-2WK'})
        ...
        if creators:
            ...
            for creator in list(set(creators)):
                creatorDict = {}
                creatorInfo = creator.find(
                    'a', {'class': re.compile((
                            '(rf-profile-item__name js-mini-profile)|(Project'
                            'OwnersInfo-userName-2oz js-mini-profile)')),
                          'href': True})
                if creatorInfo:
                    creatorName = creatorInfo.text.strip()
                    creatorName = (
                        self.sanitizeString(self.sanitizeString(creatorName)))
                ...
                location = creator.findChild(
                    'a', {'class': re.compile((
                            '(rf-profile-item__location beicons-pre beicons'
                            '-pre-location)|(ProjectOwnersInfo-userLocation-'
                            '_rE beicons-pre beicons-pre-location)')),
                          'href': True})
                ...
                if location:
                    ...
                    if link:
                        ...
                        for location in locationInfo:
                            if any(loc in location.strip().lower() for loc in
                                    ['country', 'state', 'city']):
                                loc = location.split('=')
                                creatorDict[loc[0].strip().lower()] = (
                                    loc[1].strip().replace('+', ' '))
        ...
        # tags
        tagInfo = soup.find_all(
            'a',
            {'class': re.compile('(object-tag)|(ProjectTags-tagLink-Hh_)')})
        if tagInfo:
            otherMetaData['tags'] = ','.join(
                self.sanitizeString(tag.text.strip()) for tag in tagInfo)

        # description
        description = soup.find('meta', {'property': 'og:description'})
        if description:
            desc = self.sanitizeString(
                self.validateContent('', description, 'content'))
        ...
        if popInfo:
            ...
            likes = popInfo.find(
                'div', {'class': re.compile(r'beicons-pre-thumb$')})
            views = popInfo.find(
                'div', {'class': re.compile(r'beicons-pre-eye$')})
            cmmts = popInfo.find(
                'div', {'class': re.compile(r'beicons-pre-comment$')})

            if likes:
                self.popularityMetrics['likes'] = (
                    self.sanitizeString(likes.text))

            if views:
                self.popularityMetrics['views'] = (
                    self.sanitizeString(views.text))

            if cmmts:
                self.popularityMetrics['comments'] = (
                    self.sanitizeString(cmmts.text)'''
