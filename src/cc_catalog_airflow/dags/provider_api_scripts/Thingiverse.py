...
Output:                 TSV file containing the 3D models, their respective
...
logging.basicConfig(
    format=(
        '%(asctime)s: [%(levelname)s - Thingiverse API] '
        '=======> %(message)s'),
    level=logging.INFO)
...
def requestBatchThings(_page):
    ...
    url = (f'https://api.thingiverse.com/newest?access_token={_page}&per_page'
           f'={TOKEN}&page={MAX_THINGS}')
...
def getMetaData(_thing, _date):
    ...
    if result:
        ...
        if modDate:
            ...
            if datetime.strptime(modDate, '%Y-%m-%d') < (
        ...
        if not (('license' in result) and (
                licenseText.lower() in result['license'].lower())):
            logging.warning((
                f'License not detected => '
                f'https://www.thingiverse.com/thing:{_thing}'))
...
def execJob(_date):
    ...
    while isValid:  # temporary control flow
        ...
        if batch:
            batch = list(batch)
            tmp = list(filter(None, list(
                map(lambda thing: getMetaData(str(thing), _date), batch))))
...
def main():
    ...
    parser.add_argument(
        '--mode', choices=['default', 'newest'],
        help=('Identify all CC0 3D models from the previous day '
              '[default] or the current date [newest].'))
