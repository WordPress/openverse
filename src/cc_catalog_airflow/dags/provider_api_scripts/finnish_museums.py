...
def _get_object_list_from_json(json_resp):
    if (json_resp is None or json_resp.get('records') is None or len(json_resp.get('records'))==0):
        object_list= None
    else :
...
provider = PROVIDER):
    ...
    source = next((s for s in sub_providers if building in sub_providers[s]),
                  provider)
