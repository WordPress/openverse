...
ingestion_method column to TSV files before uploading them to S3 or
PostgreSQL.
"""
import logging
import os
...
def check_and_fix_tsv_file(tsv_file_name):
    ...
    columns for the new DB schema, and attempt to add an
    ingestion_method column if the number is one short.

    It will also log a warning if the number is completely wrong.
    """
    logger.info(f'Checking for ingestion_method column in {tsv_file_name}')
    old_cols_number = 17
    ...
    if len(line_list) == old_cols_number:
        _add_ingestion_method(tsv_file_name, line_list[-1])
...
def _add_ingestion_method(tsv_file_name, source):
    ingestion_method = source if source == 'commoncrawl' else 'provider_api'
    logger.debug(f'Found source:  {source}')
    logger.info(
        f'Adding ingestion_method:  {ingestion_method} to {tsv_file_name}'
    )
    temp_tsv = tsv_file_name + '.new'
    with open(tsv_file_name, 'r') as old_tsv, open(temp_tsv, 'w') as new_tsv:
        old_line = old_tsv.readline().strip()
        while old_line:
            new_tsv.write(old_line + '\t' + ingestion_method + '\n')
            old_line = old_tsv.readline().strip()
