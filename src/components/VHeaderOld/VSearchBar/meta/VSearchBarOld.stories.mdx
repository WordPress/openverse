...
The `VSearchBarOld` component passes all attributes down to the inner
`InputField` which itself applies all its attributes to its inner `<input>`
element. So it's easy `<input>` attributes like placeholders or HTML
validations.
