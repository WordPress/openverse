...
const props = withDefaults(
  defineProps<{
     ...
     */
    features?: AudioFeature[] | readonly AudioFeature[]
    /**
     * An object of notices to display when a feature is disabled.
     * `'timestamps'`, `'duration'`, `'seek'`.
     */
    featureNotices?: Partial<Record<AudioFeature, boolean>>
    /**
  ...
  }>(),
  {
    ...
    featureNotices: () => ({} as Record<AudioFeature, boolean>),
    isTabbable: true,
    isParentSeeking: false,
  }
)

const validatePeaks = (val: unknown[]) =>
  val.every((item) => typeof item === "number")
if (props.peaks?.length) {
  if (!validatePeaks(props.peaks)) {
    console.warn("The peaks prop must be an array of numbers.")
  }
}
