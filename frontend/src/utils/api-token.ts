...
const apiTokenLogger = createConsola({
  level: 3,
  reporters: [
    {
      log(logObj) {
...
const refreshApiAccessToken = async (
  ...
  try {
    ...
    const tokenExpiry = currTimestamp() + res.data.expires_in

    apiTokenLogger.info(
      `Openverse API token refreshed, expires on ${new Date(
        tokenExpiry * 1000
      )}`
    )
...
> => {
  ...
  if (isNewTokenNeeded() && !process.fetchingMutex.isLocked()) {
    apiTokenLogger.debug("acquiring mutex lock")
    release = await process.fetchingMutex.acquire()
    apiTokenLogger.debug("mutex lock acquired, preparing token refresh request")
    process.tokenFetching = refreshApiAccessToken(apiClientId, apiClientSecret)
  }

  try {
    apiTokenLogger.debug("awaiting the fetching of the api token to resolve")
    await process.tokenFetching
    apiTokenLogger.debug("done waiting for the token, moving on now...")
  } finally {
    /**
     ...
     */
    if (release) {
      apiTokenLogger.debug("releasing mutex")
      release()
      apiTokenLogger.debug("mutex released")
    }
  }
