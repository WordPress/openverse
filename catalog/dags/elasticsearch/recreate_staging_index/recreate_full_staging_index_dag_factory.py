...
the completion of the index creation and then points the `<media_type>-full`
alias to the newly created index.

Optionally, if the `point_media_alias` param is enabled, the DAG will instead
alias the newly created index to the main media-type alias. This results in
the new index being used by default by the staging API. The index previously
aliased to `<media_type` is not deleted by default, unless the
`delete_old_index` param is enabled. Note the old index will only be deleted if
this param is enabled, _and_ the alias is pointed to the new index.
...
def recreate_full_staging_index_dag_factory(media_type: MediaType):
    ...
    with DAG(
        ...
        doc_md=__doc__,
        params={
            "point_media_alias": Param(
                default=False,
                type="boolean",
                description=(
                    f"Whether to unlink the `{media_type}`alias from the "
                    "currently aliased index, and instead alias it to the "
                    "newly created index. If `true`, the new index will be "
                    "used by the staging API and the old index will be "
                    "unlinked, but not necessarily deleted."
                ),
            ),
            "delete_old_media_index": Param(
                default=False,
                type="boolean",
                description=(
                    "Whether to delete the index previously pointed to be the "
                    f"`{media_type}` alias, if it is replaced. The index will "
        ...
        @task
        def get_target_alias(point_media_alias: bool) -> str:
            """
            Get the alias that should be linked to the newly created index.

            By default, the index will be aliased to `<media_type>-full`. If the
            `point_media_alias` param is enabled, it will instead be aliased to
            the main `<media_type>` alias.
            """
            return media_type if point_media_alias else f"{media_type}-full"
        ...
        def should_delete_index(should_delete: bool, old_index: str):
            if should_delete and old_index:
                ...
                # and we were able to find an index with the `<media_type>` alias in the
                # preceding task.
        ...
        # Get the index currently aliased by <media_type>, in case it must be deleted
        # later.
        get_current_index_if_exists = ingestion_server.get_current_index(
            media_type, http_conn_id="staging_data_refresh"
        )

        # Create the new Elasticsearch index
        do_create_index = create_index(new_index_suffix)

        # Determine the appropriate alias for the new index
        get_alias = get_target_alias(point_media_alias="{{ params.point_media_alias }}")

        # Actually point the alias
        do_point_alias = point_alias(
            target_alias=get_alias, index_suffix=new_index_suffix
        )

        check_if_should_delete_index = should_delete_index(
            should_delete="{{ params.delete_old_media_index }}",
        ...
        # Deletes the old <media-type>-aliased index after the alias has been unlinked.
        ...
        notify_complete = slack.notify_slack.override(
            ...
            text=(
                f"Finished creating full staging index {media_type}-{new_index_suffix}"
                f" aliased to {get_alias}"
            ),
        ...
        new_index_suffix >> do_create_index >> get_alias
        get_alias >> do_point_alias >> check_if_should_delete_index
