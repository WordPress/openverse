...
creating filtered indices, used by the data refreshes of each media type. It also
creates the filtered index creation DAGs for each media type using an additional
factory function.
...
The DAGs generated by the ``create_filtered_index_creation_task_groups`` can be used
...
manually, when filtered index creation is desired independently of the data
refresh. This is primarily useful in two cases: for testing changes to the filtered
