...
def _get_host_ip():
    ...
    ip_docker0 = subprocess.run(
        ["ip", "addr", "show", "docker0"], stdout=subprocess.PIPE
    )
...
class TestIngestion(unittest.TestCase):
    ...
    def clean_up():
        ...
        with upstream_db.cursor() as upstream_cur, downstream_db.cursor() as downstream_cur:
    ...
    def _wait_for_callback(self, endpoint_name="/task_done"):
        ...
        cb_listener_process = Process(
            target=run, args=(callback_listener,), kwargs=kwargs
        )
    ...
    def test05_removed_from_source_not_indexed(self):
        ...
        es_query = {"query": {"match": {"_id": id_to_check}}}
        es.indices.refresh(index="image")
        search_response = es.search(index="image", body=es_query)
...
if __name__ == "__main__":
    ...
    return_code = subprocess.call("test/generate_integration_test_docker_compose.py")
    if return_code != 0:
        print(
            "Failed to generate integration test docker-compose."
            "The existing file on disk will be used instead."
        )

    # Start Elasticsearch and database.
    print("Starting integration test datastores. . .")
    integration_compose = os.path.join(this_dir, "integration-test-docker-compose.yml")
