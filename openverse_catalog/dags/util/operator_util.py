...
def get_runner_operator(source, script_location):
    return BashOperator(
...
def get_main_runner_operator(main_function):
    return PythonOperator(
        task_id="pull_image_data",
        python_callable=main_function,
        depends_on_past=False,
...
def get_wait_till_done_operator(task_id):
    return DummyOperator(task_id=task_id, trigger_rule=TriggerRule.ALL_DONE)
