...
def get_file_staging_operator(
    dag, output_dir, minimum_file_age_minutes, identifier=TIMESTAMP_TEMPLATE
):
...
def get_table_creator_operator(dag, postgres_conn_id, identifier=TIMESTAMP_TEMPLATE):
...
def get_load_local_data_operator(
    dag, output_dir, postgres_conn_id, overwrite=False, identifier=TIMESTAMP_TEMPLATE
):
    return PythonOperator(
...
def get_copy_to_s3_operator(
    dag, output_dir, storage_bucket, aws_conn_id, identifier=TIMESTAMP_TEMPLATE
):
    return PythonOperator(
        task_id="copy_to_s3",
...
def get_one_failed_switch(dag, identifier):
...
def get_all_failed_switch(dag, identifier):
    return DummyOperator(
...
def get_one_success_switch(dag, identifier):
    return DummyOperator(
...
def get_all_done_switch(dag, identifier):
    return DummyOperator(
...
def get_file_deletion_operator(dag, output_dir, identifier=TIMESTAMP_TEMPLATE):
    return PythonOperator(
...
def get_drop_table_operator(dag, postgres_conn_id, identifier=TIMESTAMP_TEMPLATE):
    return PythonOperator(
...
def get_failure_moving_operator(dag, output_dir, identifier=TIMESTAMP_TEMPLATE):
    return PythonOperator(
...
def get_image_expiration_operator(dag, postgres_conn_id, provider):
    return PythonOperator(
