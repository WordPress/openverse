...
import provider_api_scripts.walters_art_museum as wam
import requests


RESOURCES = os.path.join(
    os.path.abspath(os.path.dirname(__file__)), "resources/waltersartmuseum"
)
...
def test_get_image_list_retries_with_none_response():
    with patch.object(
        wam.delayed_requester, "get_response_json", return_value=None
    ) as mock_get:
...
def test_get_image_list_retries_with_non_ok_response():
    ...
    with patch.object(
        wam.delayed_requester, "get_response_json", return_value=r.json()
    ) as mock_get:
        wam._get_image_list("some_class", retries=3)
...
def test_build_query_param_default():
    ...
    actual_param_made = wam._build_query_param(apikey=walters_api_key)
    expected_param = {
        "accept": "json",
...
def test_build_query_param_given():
    ...
    actual_param_made = wam._build_query_param(
        class_param=class_param, apikey=walters_api_key
    )
    expected_param = {
        "accept": "json",
        "apikey": walters_api_key,
...
def test_extract_items_list_from_json_returns_expected_output():
    ...
    actual_items_list = wam._extract_items_list_from_json(json_response_inpydict_form)
...
def test_process_image_returns_expected_output_given_right_input():
    ...
    with patch.object(wam.image_store, "add_item", return_value=100) as mock_add_item:
