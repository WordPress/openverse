...
def spark_context(request):
    conf = (
        ...
        .set(
            "spark.jars",
            (
                '../jars/hadoop-aws-2.8.1.jar,../jars/hadoop-auth-2.8.1.jar,'
                '../jars/aws-java-sdk-1.11.212.jar,../jars/postgresql-'
                '42.1.4.jar')
        )
    ...
    sc._jsc.hadoopConfiguration().set(
        ...
        os.environ['OPEN_LEDGER_ACCESS_KEY_ID']
        )
    sc._jsc.hadoopConfiguration().set(
        "fs.s3n.awsSecretAccessKey",
        os.environ['OPEN_LEDGER_SECRET_ACCESS_KEY']
    )
...
def data_by_deed(request, wat_file):
    ...
    lns = wat_file.map(lambda x: {
        ...
        'links': filter(
            lambda y: "creativecommons.org" in y['url'],
            x(
                ['Envelope']['Payload-Metadata']
...
def deed_by_domain(request, data_by_deed, sql_context):
    ...
    lns = data_by_deed.aggregateByKey(
        {}, aggregate_by_domain,
        (lambda s, d: s + d)).flatMap(lambda x: flat(x))
...
def save_to_sql(request, links, sql_context):
    links.write\
            .jdbc(
                "jdbc:postgresql://localhost:5432/commonsmapper",
                "deeds_count", mode='overwrite',
                properties={'user': 'paw', 'password': ''})
    pass
...
def test_by_domain(deed_by_domain):
    deed_by_domain.write.format('csv').save(
        "s3n://commonsmapper/licenses_by_domain/",
        mode='overwrite')
    pass
