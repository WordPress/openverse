...
class Test_CCLinks_v1(unittest.TestCase):
    ...
    def test_generateParquet(self):
        data = [[
                ...
                [
                    ...
                    (
                        '{"Images":3,"Links":{"www.westeros.org":1,'
                        '"creativecommons.org":1,"www.mediawiki.org":2}}')
                ]]
    ...
    def test_processFile_botoParams(self, mockWarcio, mockBoto3):
        self.testKey = (
            'crawl-data/CC-MAIN-2018-13/segments/1521257644271.19/'
            'wat/CC-MAIN-20180317035630-20180317055630-00006.warc.wat.gz')
        ...
        mockBoto3.resource().meta.client.head_bucket.assert_called_with(
            'commoncrawl'
            )  # check bucket
    ...
    def test_processFile_success(
            self, mockJSON, mockWarcio,
            mockGet, mockBoto3):
        self.testKey = (
        ...
        jsonData = {
            ...
            'content': {
                    ...
                    'Payload-Metadata': {
                        'HTTP-Response-Metadata': {
                            'HTML-Metadata': {
                                'Links': [{
                                    'url': (
                                        'http://newsimg.bbc.co.uk/nol/'
                                        'shared/img/v3/bbc_logo.gif'
                                        ),
                                    'alt': 'BBC News',
                                    'path': 'IMG@/src'
                                    },
                ...
                'Container': {
                    'Offset': '213390650',
                    'Filename': (
                        'CC-MAIN-20140728011800-00009-ip-10-146-231-18'
        ...
        mockGet.return_value = MagicMock(
            raw=self.warcRecord,
            url=f'https://commoncrawl.s3.amazonaws.com/{self.testKey}'
            )
    ...
    @patch('src.ExtractCCLinks.requests.get')
    @patch('src.ExtractCCLinks.ArchiveIterator')
    def test_processFile_invalidContentType(self, mockWarcio,
                                            mockGet, mockBoto3):
        self.testKey = (
            'crawl-data/CC-MAIN-2018-13/segments/1521257644271.19/'
            'wat/CC-MAIN-20180317035630-20180317055630-00011.warc.wat.gz'
            )
        self.mockWarcFiles = MagicMock()
        self.mockWarcFiles.__iter__.return_value = [self.testKey]
...
class Test_CCLinks_v2(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # init pyspark
        conf = pyspark.SparkConf().setMaster('local[*]').setAppName(
                                                        'Test_ExtractCCLinks')
    ...
    def test_processFlow(self):
        ...
        result.saveAsTextFile(
            self.cclinks.output.replace('parquet', 'text')
    ...
    def summarizeOutput(self):
        ...
        uniqueContentQuery = res.drop_duplicates(
            subset=['provider_domain', 'content_path', 'content_query_string']
            ).count()
        uniqueContent = res.drop_duplicates(
            subset=['provider_domain', 'content_path']
            ).count()

        res.registerTempTable('test_deeds')
        summary = s.sql(
            'SELECT provider_domain, count(*) AS total, '
            'count(distinct content_path) AS unique_content_path, count'
            '(distinct content_query_string) AS unique_query_string FROM test'
            '_deeds GROUP BY provider_domain ORDER BY total DESC LIMIT 100')
        summary.write.mode('overwrite').format('csv').option(
            'header', 'true'
