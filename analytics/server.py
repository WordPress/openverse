...
class SearchEventResource:
    def on_post(self, req, resp):
        j = req.media
        event_controller.create_search(query=j["query"], session_uuid=j["session_uuid"])
    ...
    def on_post(self, req, resp):
        ...
        try:
            event_controller.create_search_rating(
                query=j["query"], relevant=j["relevant"]
            )
            resp.status = falcon.HTTP_201
        except ValueError:
    ...
    def on_post(self, req, resp):
        ...
        try:
            event_controller.create_detail_event(
                event=j["event_type"], result_uuid=j["result_uuid"]
            )
            resp.status = falcon.HTTP_201
        except KeyError:
            valid_events = event_controller.list_valid_detail_events()
            resp.body = '{{"message": "Invalid event_type. Valid types: {}"}}'.format(
                valid_events
            )
            resp.status = falcon.HTTP_400
...
cors = CORS(allow_origins_list=origins, allow_all_methods=True, allow_all_headers=True)
