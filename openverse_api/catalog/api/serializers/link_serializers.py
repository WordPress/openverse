...
class ShortenedLinkResponseSerializer(Serializer):
    shortened_url = URLField(help_text="A shortened link on the `shares.cc` domain.")
...
class ShortenedLinkSerializer(ModelSerializer):
    ...
    def save(self):
        ...
        if lock:
            try:
                last_url = str(
                    ShortenedLink.objects.latest("created_on").shortened_path
                )
            ...
            shortened_link_instance = ShortenedLink(
                shortened_path=shortened_path, full_url=full_url
            )
