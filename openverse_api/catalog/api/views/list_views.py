...
class _CreateResponse(serializers.Serializer):
    url = serializers.HyperlinkedRelatedField(
        view_name="list-detail", read_only=True, help_text="The URL of the new list."
    )
...
class CreateList(_List):
    @swagger_auto_schema(
        operation_id="list_create", responses={201: _CreateResponse, 400: "Bad Request"}
    )
    ...
    def post(self, request, format=None):
        ...
        return Response(status=201, data={"url": url, "auth": _list.auth})


class ListDetail(_List, RetrieveModelMixin):
    @swagger_auto_schema(
        operation_id="list_detail",
        responses={200: ImageListResponseSerializer, 404: "Not Found"},
    )
    ...
    def _authenticated(list_model, request):
        if "HTTP_AUTHORIZATION" not in request.META:
            return False
        return list_model.auth == request.META["HTTP_AUTHORIZATION"].split(" ")[1]

    @swagger_auto_schema(
        operation_id="list_delete",
        security=[
            {"list key": {"type": "apiKey", "name": "api_key", "in": "header"}},
        ],
        responses={204: "", 403: "Forbidden", 404: "Not Found"},
    )
