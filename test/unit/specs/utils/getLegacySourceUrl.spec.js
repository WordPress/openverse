...
describe('getLegacySourceUrl', () => {
  it('returns a url for each audio source', () => {
    ...
    const audioSources = Object
      .keys(legacySourceMap)
      .filter(sourceName => legacySourceMap[sourceName].audio);
    const getAudioSourceUrl = getLegacySourceUrl('audio');
    const urls = audioSources.map(sourceName => getAudioSourceUrl(sourceName, search));
  ...
  it('returns a url for each video source', () => {
    ...
    const audioSources = Object
      .keys(legacySourceMap)
      .filter(sourceName => legacySourceMap[sourceName].video);
    const getVideoSourceUrl = getLegacySourceUrl('video');
    const urls = audioSources.map(sourceName => getVideoSourceUrl(sourceName, search));
  ...
  it('returns a url for each image source', () => {
    ...
    const audioSources = Object
      .keys(legacySourceMap)
      .filter(sourceName => legacySourceMap[sourceName].image);
    const getImageSourceUrl = getLegacySourceUrl('image');
    const urls = audioSources.map(sourceName => getImageSourceUrl(sourceName, search));
