...
| [`recreate_full_audio_staging_index`](#recreate_full_audio_staging_index) | `None`            |
| [`recreate_full_image_staging_index`](#recreate_full_image_staging_index) | `None`            |
...
1.  [`recreate_full_audio_staging_index`](#recreate_full_audio_staging_index)
1.  [`recreate_full_image_staging_index`](#recreate_full_image_staging_index)
...
### `recreate_full_audio_staging_index`
...
`REINDEX` action to create a new index in the staging elasticsearch cluster for
the given media type, suffixed by the current timestamp. The DAG awaits the
completion of the index creation and then points the `<media_type>-full` alias
to the newly created index.

Optionally, if the `point_media_alias` param is enabled, the DAG will instead
alias the newly created index to the main media-type alias. This results in the
new index being used by default by the staging API. The index previously aliased
to `<media_type` is not deleted by default, unless the `delete_old_index` param
is enabled. Note the old index will only be deleted if this param is enabled,
_and_ the alias is pointed to the new index.

Indices created using this DAG with the `<media_type>-full` alias are never
automatically deleted. They should be managed manually.
...
### `recreate_full_image_staging_index`
