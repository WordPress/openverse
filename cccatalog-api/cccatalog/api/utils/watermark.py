...
def print_attribution_for_image_on_frame(image, frame):
    ...
    placeholder_text = "Released to Public: Space Shuttle Discovery Catches " \
                       "a Ride by Lori Losey/NASA, August 19, 2005 (NASA))" \
                       "\npingnews.com\nCC pdm 1.0"
    draw.text(
...
for image_path in images:
    _img = open_image(image_path)
    _frame = create_frame_for_image(_img)
    place_image_inside_frame(_img, _frame)
    print_attribution_for_image_on_frame(_img, _frame)
    _frame.show()
